      ...
      {stats && (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          ...
          <div className="rounded-lg border bg-card p-6">
            ...
            <div
              className={`mt-2 text-2xl font-bold ${stats.totalPnl >= 0 ? 'text-green-500' : 'text-red-500'}`}
            >
              {stats.totalPnl >= 0 ? '+' : ''}
          ...
          <div className="rounded-lg border bg-card p-6">
            ...
            <div
              className={`mt-2 text-2xl font-bold ${stats.avgPnl >= 0 ? 'text-green-500' : 'text-red-500'}`}
            >
              {stats.avgPnl >= 0 ? '+' : ''}
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {positions.map((position) => (
                <div key={position.id} className="rounded-lg border bg-card p-6">
        ...
        <DialogContent>
          ...
          {positionToClose && (
            <div className="space-y-4">
              ...
              <div
                className={`p-4 rounded-lg border ${positionToClose.pnl >= 0 ? 'bg-green-500/10 border-green-500/20' : 'bg-red-500/10 border-red-500/20'}`}
              >
                <div className="text-sm text-muted-foreground">Unrealized P&L</div>
                <div
                  className={`text-lg font-bold ${positionToClose.pnl >= 0 ? 'text-green-500' : 'text-red-500'}`}
                >
                  {positionToClose.pnl >= 0 ? '+' : ''}
                  {positionToClose.pnl.toFixed(2)}%
                </div>
              </div>
            </div>
          )}
