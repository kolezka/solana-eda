...
/**
 * Meteora DEX Client
 * Note: Meteora DLMM SDK integration requires specific setup.
 * This client provides a fallback to Jupiter API with Meteora routing.
 */
export class MeteoraClient implements DEXClient {
  ...
  // Meteora program IDs
  private static readonly METEORA_PROGRAM_ID = 'LBTyrCj6p4t3YqHdNMK3pJ8W9h5Mj';
  ...
  /**
   * Get a quote using Jupiter API with Meteora routing
   */
  async getQuote(
    inputMint: string,
    outputMint: string,
    ...
    try {
      ...
      const hasMeteoraRoute = quoteResponse.routePlan?.some((step) =>
        step.swapInfo?.label?.toLowerCase().includes('meteora')
      );

      if (!hasMeteoraRoute) {
        console.warn('[MeteoraClient] No Meteora route found, returning best available quote');
      }
      ...
      console.error('[MeteoraClient] Error getting quote:', error);
      throw error;
    }
  }

  /**
   * Execute a swap using Jupiter API (which will route through Meteora if available)
   */
  async executeSwap(
    quote: DEXQuote,
    ...
    try {
      ...
      console.error('[MeteoraClient] Error executing swap:', error);
...
export type MeteoraQuote = DEXQuote;
